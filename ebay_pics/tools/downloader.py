...
class Downloader:
    ...
    @time_execution
    def download(self, links, headers=None):
        args = self._prepare_args(links, headers)
    ...
    def _run_download(args):
        ...
        link, headers = args
        try:
            return requests.get(link, headers=headers, timeout=20).content
        except requests.RequestException as err:
            logger.error('Problem with {}'.format(link))
            logger.exception(err)

    @staticmethod
    def _prepare_args(links, headers):
        args = []
        if isinstance(links, list):
            for item in links:
                args.append((item, headers))
        else:
            args.append((links, headers))
        return args
